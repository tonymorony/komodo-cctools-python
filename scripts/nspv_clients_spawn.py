...
j = i + nspv_clients_to_start
for i in range(nspv_clients_to_start):
    j = j + 1
    subprocess.call(['./komodod', '-ac_name=ZEXO', '-conf=' + sys.path[0] + '/node_' + str(i) + "/" + ac_name + ".conf",
                     '-port=' + str(7000 + i), '-rpcport=' + str(7000 + j), '-datadir=' + sys.path[0] + '/node_' + str(i),
                     ...
                     '-ac_public=1', '-nSPV=1', '-addnode=95.217.44.58', '-daemon'])
